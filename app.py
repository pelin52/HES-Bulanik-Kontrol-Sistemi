...
class EndustriyelBarajKontrolMotoru:
    ...
    def bulanik_mimarini_kur(self):
        # Dilsel Tanımlamalar: 0:Düşük, 1:Orta, 2:Yüksek
...
class HESProDashboard:
    def __init__(self, root):
        ...
        self.aciklamalar = [
            "Yüksek Doluluk + Yüksek Debi -> Maksimum Üretim",
            "Yüksek Doluluk + Orta Debi -> Yüksek Üretim",
            "Orta Doluluk + Yüksek Debi -> Yüksek Üretim",
            "Düşük Doluluk + Yüksek Debi -> Orta Üretim",
            "Yüksek Doluluk + Yüksek Fiyat -> Maksimum Kâr Modu",
            "Orta Doluluk + Yüksek Fiyat -> Yüksek Kâr Modu",
            "Orta Doluluk + Orta Fiyat -> Nominal Üretim",
            "Düşük Doluluk + Yüksek Fiyat -> Orta Üretim",
            "Düşük Doluluk + Düşük Fiyat -> Minumum Üretim",
            "Düşük Doluluk + Orta Fiyat -> Düşük Üretim",
            "Orta Doluluk + Düşük Fiyat -> Düşük Üretim",
            "Düşük Debi + Düşük Fiyat -> Bekleme Modu",
            "Orta Doluluk + Orta Debi -> Nominal Üretim",
            "Orta Debi + Orta Fiyat -> Nominal Üretim",
        ...
        # Butonlar Paneli
        self.btn_analiz = tk.Button(self.control_frame, text="MANUEL ANALİZ ET", command=self.hesapla, bg="#4e4e4e", fg="white", 
                  activebackground="#3a3a3a", activeforeground="white", font=("Segoe UI", 10, "bold"), bd=0, cursor="hand2", pady=6, padx=10)
        self.btn_analiz.pack(fill=tk.X, padx=15, pady=(15,5))

        self.btn_sim = tk.Button(self.control_frame, text="CANLI SİMÜLASYONU BAŞLAT", command=self.toggle_simulasyon, bg="#007acc", fg="white", 
                  activebackground="#005999", activeforeground="white", font=("Segoe UI", 10, "bold"), bd=0, cursor="hand2", pady=8, padx=10)
    ...
    def animasyon_dongusu(self):
        ...
        # Dinamik Doğa ve Piyasa Dalgalanması Algoritmaları
        mevcut_doluluk = max(0, min(100, self.s1.get() + random.choice([-1, 0, 1])))
        mevcut_debi = max(0, min(1000, self.s2.get() + random.randint(-40, 40)))
        
        # Fiyat döngüsü gece-gündüz simülasyonu mantığında dalgalanır
        mevcut_fiyat = max(0, min(5000, self.s3.get() + random.randint(-200, 200)))
        
        self.s1.set(mevcut_doluluk)
        self.s2.set(mevcut_debi)
        self.s3.set(mevcut_fiyat)
    ...
    def hesapla(self):
        ...
        # Finansal Analiz Hesap Modeli: (Maksimum Güç: 120 MW tabanlı hidrolik formülasyon)
        anlik_guc = (deb_val / 1000.0) * (cikis / 100.0) * 120.0
        # Saatlik üretim geliri hesabı (Saniyede bir güncellendiği için saniyelik orantılama)
        ...
        # Alarm Denetleme Tetikçisi
        self.alarm_kontrol(d_val, deb_val, cikis)

        # SCADA Grafik Çizimi Güncellemesi
        self.ax.clear()
        ...
        self.ax.set_title(f"Bulanık Kontrol Çıktı Kümesi Görselleştirmesi", color="white", fontsize=11, fontweight='bold', pad=10)
        ...
        if aktif_kural_sayisi == 0:
            self.log.insert(tk.END, " ⚠ UYARI: Mevcut girdi parametrelerine uygun aktif kural bulunamadı (Eşik: 0.05).\n")
    ...
    def create_custom_scale(self, label, min_v, max_v):
        ...
        lbl = tk.Label(frame, text=label, bg="#2d2d2d", fg="white", font=("Segoe UI", 10))
        lbl.pack(anchor="w")
        
        scale = tk.Scale(frame, from_=min_v, to=max_v, orient='horizontal', bg="#2d2d2d", fg="#aaaaaa",
                         troughcolor="#1e1e1e", highlightbackground="#2d2d2d", activebackground="#007acc", bd=0, font=("Segoe UI", 9))
        scale.pack(fill=tk.X, pady=(2,0))
        return scale

    def setup_rules_tab(self):
        lbl_info = tk.Label(self.tab_rules, text="Sistemde Tanımlı Sabit Bulanık Mantık Kuralları (Toplam: 15 Kural)", bg="#1e1e1e", fg="white", font=("Segoe UI", 11, "bold"))
        lbl_info.pack(anchor="w", padx=15, pady=10)
        
        columns = ("kural_no", "kural_tanimi", "cikis_katman")
        self.tree = ttk.Treeview(self.tab_rules, columns=columns, show="headings", height=16)
        
        self.tree.heading("kural_no", text="Kural ID")
        self.tree.heading("kural_tanimi", text="Eğer Koşulu (Giriş Mantığı)")
        self.tree.heading("cikis_katman", text="O Halde (HES Aksiyonu)")
        
        self.tree.column("kural_no", width=80, anchor="center")
        self.tree.column("kural_tanimi", width=500, anchor="w")
        self.tree.column("cikis_katman", width=180, anchor="center")
        
        style = ttk.Style()
        style.configure("Treeview", background="#252526", foreground="white", fieldbackground="#252526", rowheight=25, font=("Segoe UI", 10))
        ...
        for i, aciklama in enumerate(self.aciklamalar):
            ...
            self.tree.insert("", tk.END, values=(f"Kural {i+1}", condition, action))
